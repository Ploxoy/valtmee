...
const forcedLabels = [
  { includes: ["mohammadi"], label: "Mohammadi" },
  { includes: ["asiel", "den haag"], label: "asieldebat Den Haag" },
  { includes: ["ijsselstein", "noodopvang"], label: "noodopvang IJsselstein" },
  { includes: ["ijsselstein", "vuurwerk"], label: "vuurwerk IJsselstein" },
];
...
type ScoredTopic = Omit<Candidate, "articleIds" | "sources" | "kinds"> & {
  sources: string[];
  kinds: string[];
};
...
function extractCapitalizedPhrases(title: string) {
  ...
  return (matches ?? [])
    ...
    .filter((phrase) => {
      ...
      if (weakPhraseStarts.has(firstWord)) return false;
      if (!phrase.includes(" ")) {
...
function extractAcronyms(title: string) {
...
function strongestContextWords(title: string) {
...
function chooseEntity(title: string) {
...
function eventLabelFromArticle(article: Article) {
  ...
  if (!title || contexts.length === 0) return null;
  ...
  if (contexts.includes("miami")) return "GP Miami";
  if (contexts.includes("brand") && entity) return compactLabel(`brand ${entity}`);
  if (contexts.includes("noodopvang") && entity) {
    return compactLabel(`noodopvang ${entity}`);
  }

  if (contexts.includes("protest") && entity) return compactLabel(`protest ${entity}`);
  if (contexts.includes("verkeersongeval") && entity) {
    return compactLabel(`verkeersongeval ${entity}`);
  }

  if (entity && contexts.length > 0) return compactLabel(`${contexts[0]} ${entity}`);

  const cleanPart = parts
    .map((part) => tokenize(part).filter(isUsefulWord))
    .find((words) => words.length >= 2);

  if (cleanPart) return cleanPart.slice(0, 3).join(" ");
...
function addCandidate(
  ...
  if (!topic || weakTopicWords.has(topic)) return;
...
function articleFromItem(item: string, source: string, index: number): Article {
  return {
    id: `${source}:${index}`,
    source,
...
function topicBoost(topic: string) {
...
function scoreTopics(articles: Article[]): ScoredTopic[] {
...
function mergeRelatedTopics(topics: ScoredTopic[]) {
  const merged: ScoredTopic[] = [];
  let ijsselsteinTopic: ScoredTopic | null = null;
  ...
  for (const topic of topics) {
    if (topic.topic.includes("ijsselstein")) {
      if (!ijsselsteinTopic) {
        ...
        ijsselsteinTopic.sources = [
          ...new Set([...ijsselsteinTopic.sources, ...topic.sources]),
        ].sort();
        ijsselsteinTopic.examples = [
          ...new Set([...ijsselsteinTopic.examples, ...topic.examples]),
        ].slice(0, 2);
      }
  ...
  if (ijsselsteinTopic) merged.push(ijsselsteinTopic);

  return merged.sort(
    (a, b) => b.score - a.score || b.sources.length - a.sources.length
  );
}

function finalTopics(topics: ScoredTopic[], limit = 5) {
  return mergeRelatedTopics(topics)
    .filter((topic) => {
...
async function getOphef() {
  ...
  for (const result of results) {
    ...
    sourceNames.push(result.value.name);
    articles.push(
  ...
  const topics = finalTopics(scoreTopics(articles));
...
export async function GET() {
  return Response.json(await getOphef());
}
